...
import { Power, Music, ChevronRight, Flame, Heart, Film, Users } from 'lucide-react';

// ------------------------------------------
// 1. LIGHT SWITCH STAGE
// ------------------------------------------
export function LightSwitch({ onTurnOn }) {
  return (
    <motion.div exit={{ opacity: 0, transition: { duration: 1.5 } }} className="flex flex-col items-center text-white z-50">
      ...
      <div className="w-16 h-40 bg-gray-900 rounded-full p-2 relative shadow-inner border border-gray-800">
        <motion.div drag="y" dragConstraints={{ top: 0, bottom: 90 }} dragElastic={0.1}
          onDragEnd={(e, info) => { if (info.offset.y > 60) onTurnOn(); }}
          className="w-12 h-12 bg-gray-300 rounded-full flex items-center justify-center cursor-grab shadow-lg"
        >
...
// 2. MUSIC VINYL STAGE
// ------------------------------------------
export function MusicVinyl({ isPlaying, onPlayMusic }) {
  return (
    <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1, transition: { duration: 1 } }} exit={{ scale: 1.2, opacity: 0, transition: { duration: 1 } }} className="flex flex-col items-center z-20">
      ...
      <motion.div drag="x" dragConstraints={{ left: 0, right: 0 }}
        onDragEnd={(e, info) => { if (Math.abs(info.offset.x) > 50) onPlayMusic(); }}
        animate={{ rotate: isPlaying ? 360 : 0 }}
        transition={{ repeat: isPlaying ? Infinity : 0, duration: 4, ease: "linear" }}
        className="w-40 h-40 bg-black rounded-full border-4 border-gray-800 flex items-center justify-center shadow-2xl cursor-grab"
      >
...
// ------------------------------------------
// 3. DECORATE SLIDER STAGE
// ------------------------------------------
export function DecorSlider({ onDecorate }) {
  return (
    <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1, transition: { duration: 1 } }} exit={{ scale: 1.2, opacity: 0, transition: { duration: 1 } }} className="flex flex-col items-center z-20">
      ...
      <div className="w-64 h-14 bg-white rounded-full p-1 relative shadow-lg flex items-center overflow-hidden border border-gray-200">
        ...
        <motion.div drag="x" dragConstraints={{ left: 0, right: 200 }} dragElastic={0.1}
          onDragEnd={(e, info) => { if (info.offset.x > 150) onDecorate(); }}
          className="w-12 h-12 bg-rose-400 rounded-full flex items-center justify-center z-10 cursor-grab shadow-md transition-all"
        >
...
// ------------------------------------------
// 4. CAKE & BALLOONS STAGE (Includes Poetic Message)
// ------------------------------------------
export function CakeAndBalloons({ stage, onLightCandle, onSequenceComplete }) {
  const bgBalloons = Array.from({ length: 30 }).map((_, i) => ({
    id: i, xOffset: (i * 17) % 100, delay: (i * 0.3) % 3, duration: 7 + (i % 5),
    color: ['bg-rose-400', 'bg-blue-400', 'bg-yellow-400', 'bg-green-400', 'bg-purple-400'][i % 5],
    textColor: ['text-rose-400', 'text-blue-400', 'text-yellow-400', 'text-green-400', 'text-purple-400'][i % 5]
  }));

  const letterBalloons = [
    { char: 'R', color: 'bg-rose-500', textColor: 'text-rose-500' },
    { char: 'O', color: 'bg-blue-500', textColor: 'text-blue-500' },
    { char: 'M', color: 'bg-yellow-500', textColor: 'text-yellow-500' },
    { char: 'A', color: 'bg-purple-500', textColor: 'text-purple-500' }
  ];
  ...
  return (
    <>
      ...
      <div className="absolute top-24 flex gap-3 z-10">
        {letterBalloons.map((b, i) => (
          <motion.div key={`let-${i}`} initial={{ y: '100vh', opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 2, ease: "easeOut", delay: 1 + (i * 0.5) }} className="relative flex flex-col items-center">
            <div className={`w-14 h-20 rounded-[50%] flex items-center justify-center text-white font-extrabold text-2xl shadow-xl border border-white/30 ${b.color}`}>
              {b.char}
              <div className={`absolute -bottom-2 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[6px] border-r-[6px] border-b-[8px] border-transparent border-b-current ${b.textColor}`}></div>
            </div>
            <div className="w-[1px] h-20 bg-gray-400 opacity-50"></div>
          </motion.div>
        ))}
      </div>
      ...
      {stage === 3 && (
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0, transition: { duration: 1.5, delay: 0.5 } }} exit={{ opacity: 0, transition: { duration: 1 } }} className="absolute bottom-28 flex flex-col items-center z-20">
          ...
          <div className="w-20 h-40 relative flex flex-col items-center justify-end border-2 border-dashed border-gray-400/50 rounded-xl p-2 bg-white/30 backdrop-blur-sm">
            ...
            <motion.div drag="y" dragConstraints={{ top: -100, bottom: 0 }} dragElastic={0.1}
              onDragEnd={(e, info) => { if (info.offset.y < -50) onLightCandle(); }}
              className="w-4 h-16 bg-yellow-700 rounded-sm relative cursor-grab z-30 flex flex-col items-center"
            >
      ...
      {stage >= 4 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { duration: 1.5 } }} className="absolute inset-0 flex flex-col items-center justify-center z-30 pointer-events-none">
          ...
          {stage === 4 && <MessageSequence onComplete={onSequenceComplete} />}
        </motion.div>
      )}
    </>
  );
...
function MessageSequence({ onComplete }) {
  ...
  const messages = [
    "Today is...", "As beautiful as other days", "But you realize...", "Another year has gone", "Do you know?", "Today is just special...", "Because of YOU, Rom Rom ❤️"
  ];

  useEffect(() => {
    if (msgIndex < messages.length) {
      const timer = setTimeout(() => setMsgIndex(prev => prev + 1), 4000); 
  ...
  return (
    <div className="absolute bottom-20 w-full text-center px-6 pointer-events-none z-50">
      <AnimatePresence mode="wait">
        <motion.p key={msgIndex} initial={{ opacity: 0, y: 10, filter: "blur(4px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)", transition: { duration: 1.5, ease: "easeOut" } }} exit={{ opacity: 0, y: -10, filter: "blur(4px)", transition: { duration: 1.5, ease: "easeIn" } }} className="text-2xl font-serif font-bold text-gray-800 italic tracking-wide drop-shadow-md">
          {messages[msgIndex]}
        </motion.p>
      </AnimatePresence>
    </div>
...
// ------------------------------------------
// 5. MEMORY SWIPE STAGE
// ------------------------------------------
export function MemorySwipe({ onComplete, buzz }) {
  const [polaroidIndex, setPolaroidIndex] = useState(0);
  const polaroids = [
    { id: 1, text: "Bandariya ki pehli photo 🐒", color: "bg-rose-100" },
    { id: 2, text: "Jab tu emotional hoti hai 😂", color: "bg-blue-100" },
    { id: 3, text: "Mera favourite headache ❤️", color: "bg-purple-100" }
  ];
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { duration: 1.5 } }} exit={{ opacity: 0, transition: { duration: 1.5 } }} className="absolute inset-0 w-full h-full flex flex-col items-center justify-center z-40 bg-[#fff5f5]">
      ...
      <div className="relative w-full max-w-[280px] h-[380px]">
        {polaroids.map((card, index) => {
          ...
          return (
            <motion.div key={card.id} drag={isTop ? "x" : false} dragConstraints={{ left: 0, right: 0 }}
              onDragEnd={(e, info) => {
                if (Math.abs(info.offset.x) > 100) {
                  setPolaroidIndex(prev => prev + 1);
                  buzz('light');
                  if(index === polaroids.length - 1) setTimeout(onComplete, 2000);
                }
              }}
              initial={{ scale: 1 - (index - polaroidIndex) * 0.05, y: (index - polaroidIndex) * 10, opacity: 0 }}
              animate={{ scale: 1 - (index - polaroidIndex) * 0.05, y: (index - polaroidIndex) * 15, zIndex: 10 - index, opacity: 1, transition: { duration: 0.8 } }}
              exit={{ x: info => info.offset?.x > 0 ? 300 : -300, opacity: 0, transition: { duration: 0.5 } }}
              className={`absolute inset-0 w-full h-full p-4 pb-10 bg-white rounded-xl shadow-2xl border border-gray-100 flex flex-col ${isTop ? 'cursor-grab active:cursor-grabbing' : ''}`}
              style={{ rotate: isTop ? 0 : (index % 2 === 0 ? 3 : -3) }}
            >
              <div className={`w-full flex-1 rounded-lg mb-4 flex items-center justify-center text-gray-500 ${card.color}`}>[ Photo {card.id} ]</div>
              <p className="text-center text-gray-800 font-medium font-serif">{card.text}</p>
...
// 6. FILM ROLL STAGE
// ------------------------------------------
export function FilmRoll({ onComplete }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { duration: 2 } }} exit={{ opacity: 0, transition: { duration: 2 } }} className="absolute inset-0 w-full h-full flex flex-col items-center justify-center z-50 bg-black">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0, transition: { duration: 1, delay: 1 } }} className="absolute top-16 text-white/50 tracking-widest text-sm flex items-center gap-2">
        <Film className="w-4 h-4" /> PLAYING MEMORIES
      </motion.div>
      <div className="relative w-full h-[60vh] overflow-hidden flex items-center">
        <motion.div className="flex w-[300vw] h-full" animate={{ x: `-${activeVid * 100}vw` }} transition={{ type: "tween", ease: "easeInOut", duration: 1.5 }}>
          {[1, 2, 3].map((vid) => (
            <div key={vid} className="w-[100vw] h-full flex items-center justify-center px-6">
              <div className="w-full h-[80%] bg-gray-900 border-t-[20px] border-b-[20px] border-black relative shadow-2xl flex items-center justify-center overflow-hidden">
                <div className="absolute top-[-16px] w-full flex justify-around opacity-30 z-20">{[...Array(10)].map((_, i) => <div key={`t-${i}`} className="w-4 h-3 bg-white rounded-sm"></div>)}</div>
                <div className="absolute bottom-[-16px] w-full flex justify-around opacity-30 z-20">{[...Array(10)].map((_, i) => <div key={`b-${i}`} className="w-4 h-3 bg-white rounded-sm"></div>)}</div>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-white/40 text-lg">[ Auto Playing Video {vid} ]</span>
                </div>
              </div>
            </div>
          ))}
...
// ------------------------------------------
// 7. FOGGY FINALE STAGE
// ------------------------------------------
export function FoggyFinale({ buzz }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { duration: 2 } }} className="absolute inset-0 w-full h-full flex flex-col items-center justify-center z-50 bg-[#fff5f5]">
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1, transition: { delay: 0.5, duration: 1.5 } }} className="relative w-[90%] max-w-[300px] h-[400px] bg-white p-3 rounded-3xl shadow-2xl touch-none">
        <div className="absolute inset-3 rounded-2xl overflow-hidden bg-black flex items-center justify-center text-white/50">[ Group Photo Here ]</div>
      ...
      <AnimatePresence>
        {isRubbed && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0, transition: { duration: 1 } }} className="mt-8 text-center w-full px-6">
            ...
            <button onClick={() => confetti({ particleCount: 300, spread: 150, origin: { y: 0.6 }, colors: ['#f43f5e', '#fb923c', '#3b82f6'] })} className="w-full bg-gradient-to-r from-rose-400 to-orange-400 text-white py-3 rounded-xl font-bold shadow-xl active:scale-95 transition-transform duration-300">
              Celebrate! 🎁
            </button>
          </motion.div>
        )}
